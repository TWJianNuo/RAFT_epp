...
class Logger:
    ...
    def write_vls(self, data_blob, outputs, flowselector, step):
        ...
        depthpredvls = tensor2disp(1 / outputs[('depth', 4)], vmax=0.15, viewind=0)
        depthinvls = tensor2disp(1 / data_blob['mdDepth_pred'], vmax=0.15, viewind=0)

        img_val_up = np.concatenate([np.array(insvls), np.array(img2)], axis=1)
        img_val_mid2 = np.concatenate([np.array(depthinvls), np.array(depthpredvls)], axis=1)
