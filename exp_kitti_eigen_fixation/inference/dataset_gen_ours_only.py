...
class KITTI_eigen(data.Dataset):
    def __init__(self, entries, inheight, inwidth, maxinsnum, root, ins_root, depth_root, depthvls_root, mdPred_root=None,
                 prediction_root=None, istrain=True, muteaug=False, isgarg=False, banremovedup=False, deepv2dpred_root=None, flowPred_root=None, RANSACPose_root=None, baninsmap=False, bandepth=False):
        ...
        for entry in self.remove_dup(entries):
            ...
            if not os.path.exists(depthpath) and not self.bandepth:
                continue
    ...
    def __getitem__(self, index):
        ...
        data_blob = self.wrapup(img1=img1, img2=img2, flowgt=flowgt, flowgt_vls=flowgt_vls, depthmap=depth, depthvls=depthvls, depthpred=depthpred,
                                depthpred_deepv2d=depthpred_deepv2d, mdDepth_pred=mdDepth_pred, intrinsic=intrinsic, insmap=inspred, flowpred=flowpred_RAFT, rel_pose=rel_pose, posepred=posepred,
                                posepred_deepv2d=posepred_deepv2d, tag=self.entries[index], size_str=size_str)
        return data_blob
    ...
    def wrapup(self, img1, img2, flowgt, flowgt_vls, depthmap, depthvls, depthpred,
               depthpred_deepv2d, mdDepth_pred, intrinsic, insmap, flowpred, rel_pose, posepred, posepred_deepv2d, tag, size_str):
