...
class EppFlowNet(nn.Module):
    ...
    def extract_averaged_residual_depth(self, insmap, residual_depth):
        maxinsnum = insmap.max().item() + 1
        assert maxinsnum > 0
        insnum = self.eppcompress(insmap, (insmap >= 0).squeeze(1).unsqueeze(-1).unsqueeze(-1).float(), maxinsnum)

        residual_depth_sum = self.eppcompress(insmap, residual_depth.squeeze(1).unsqueeze(-1).unsqueeze(-1).float(), maxinsnum)
        residual_depth_mean = residual_depth_sum / (insnum + 1e-10)
        residual_depth_mean_inf = self.eppinflate(insmap, residual_depth_mean).squeeze(-1).squeeze(-1).unsqueeze(1)
        return residual_depth_mean_inf
    ...
    def forward(self, image1, image2, depthpred, intrinsic, posepred, insmap):
        ...
        residual_depth_mean1 = self.extract_averaged_residual_depth(insmap, residual_depth1)
        residual_depth_mean2 = self.extract_averaged_residual_depth(insmap, residual_depth2)

        depth1 = torch.exp(torch.log(depthpred) + residual_depth_mean1)
        depth2 = torch.exp(torch.log(depthpred) + residual_depth_mean2)
