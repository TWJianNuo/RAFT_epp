...
@torch.no_grad()
def validate_kitti(models, args, eval_loader, group, isdeepv2dpred=False, isbackground=True, isvls=True, iters=1):
    ...
    for val_id, data_blob in enumerate(tqdm(eval_loader)):
        ...
        for k in range(iters):
            outputs = models['model_pose'].module(image1, image2, depthpred, intrinsic, posepred, insmap)
        ...
        outputs = models['model_depth'].module(image1, image2, depthpred, intrinsic, posepred, insmap)
...
def train(gpu, ngpus_per_node, args):
    ...
    for key in keys:
        ...
        if args.restore_ckpt_pose is not None and args.restore_ckpt_depth is not None:
            if key == 'model_depth':
                restore_ckpt = args.restore_ckpt_depth
            else:
                restore_ckpt = args.restore_ckpt_pose
            print("=> loading checkpoint '{}'".format(restore_ckpt))
            loc = 'cuda:{}'.format(args.gpu)
            checkpoint = torch.load(restore_ckpt, map_location=loc)
    ...
    validate_kitti(models, args, eval_loader, group, isdeepv2dpred=True, isbackground=True, isvls=False)
    return
...
if __name__ == '__main__':
    ...
    parser.add_argument('--restore_ckpt_depth', help="restore checkpoint")
    parser.add_argument('--restore_ckpt_pose', help="restore checkpoint")
