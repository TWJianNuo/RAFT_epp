...
from EppflowLEAStereo.models.cell_level_search_2d import Cell
from EppflowLEAStereo.models.genotypes_2d import PRIMITIVES
from EppflowLEAStereo.models.operations_2d import *
from EppflowLEAStereo.models.decoding_formulas import Decoder
...
class Disp(nn.Module):
    ...
    def forward(self, x, nedges):
        bz, _, ndepths, featureh, featurew = x.shape
        x = torch.squeeze(x, 1)
        x = self.softmax(x)
        x = x * nedges.expand([bz, -1, featureh, featurew])
        x = torch.sum(x, dim=1, keepdim=True)
        x = F.interpolate(x, [int(featureh*3), int(featurew*3)], mode='bilinear', align_corners=False)
        return x


class AutoFeature(nn.Module):
    def __init__(self, num_layers, filter_multiplier=8, block_multiplier=4, step=4, cell=Cell):
