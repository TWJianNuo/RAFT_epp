...
class VirtualKITTI2(data.Dataset):
    def __init__(self, args, isapproxpose, inheight, inwidth, root='datasets/KITTI', entries=None, istrain=True):
    ...
    def __getitem__(self, index):
        while True:
            try:
                ...
                index = np.mod(index + 1, len(self.entries))
        ...
        pMImg = self.cpuinflate(renamed_ins, intrinsic, renamed_poses)
        tag = self.get_tag(index)
        data_blob = self.wrapup(img1, img2, flowmap, depthmap, intrinsic, renamed_ins, renamed_poses, renamed_ang, renamed_scale, pMImg, tag)
        return data_blob

    def cpuinflate(self, renamed_ins, intrinsic, renamed_poses):
        h, w = renamed_ins.shape
        pMImg = np.zeros([h, w, 4, 4])
        pM = intrinsic @ renamed_poses @ np.linalg.inv(intrinsic)
        for k in np.unique(renamed_ins):
            if k == -1:
                pMImg[renamed_ins == k] = np.eye(4)
            else:
                pMImg[renamed_ins == k] = pM[k]
        return pMImg

    def __len__(self):
        return len(self.entries)

    def wrapup(self, img1, img2, flowmap, depthmap, intrinsic, renamed_ins, renamed_poses, renamed_ang, renamed_scale, pMImg, tag):
        ...
        pMImg = torch.from_numpy(pMImg).unsqueeze(0).float()
