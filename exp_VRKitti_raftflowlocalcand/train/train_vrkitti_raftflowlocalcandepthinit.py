...
from exp_VRKitti_raftflowlocalcand.core_raftflowlocalcand import MDiRaft
import copy

from exp_VRKitti_raftflowlocalcand.dataset_VRKitti2 import VirtualKITTI2
...
class Logger:
    ...
    def write_vls(self, data_blob, flowpred, mDdoutputs, selector, step, X=None):
        ...
        depthpredvls = tensor2disp(1 / mDdoutputs[('mDepth', 0)], vmax=0.15, viewind=0)
        ...
        img_val_mid = np.concatenate([np.array(depthgtvls), np.array(depthpredvls)], axis=1)
        img_val = np.concatenate([np.array(img_val_up), np.array(img_val_mid), np.array(img_val_down)], axis=0)
        self.writer.add_image('predvls', (torch.from_numpy(img_val).float() / 255).permute([2, 0, 1]), step)

        if X is not None:
            X = X[:, :, 0:3]
            self.writer.add_image('Sampling', (torch.from_numpy(X).float() / 255).permute([2, 0, 1]), step)

    def write_vls_eval(self, data_blob, flowpred, mDdoutputs, selector, evalidx, step):
        img1 = data_blob['img1'][0].permute([1, 2, 0]).numpy().astype(np.uint8)
        insmap = data_blob['insmap'][0].squeeze().numpy()
    ...
    def vls_sampling(self, image1, image2, depthgt, initflow, pMImg, flowgt, flowpred, coords_lvl_vls, prob_vls):
        ...
        selector = (depthgtnp > 0) * (depthgtnp < 30) * (np.mod(xx, dsr) == 0) * (np.mod(yy, dsr) == 0)
        ...
        rndP = pMImg[0, 0, rndy, rndx, :, :].cpu().numpy()

        rnddsx = int(rndx / dsr)
        rnddsy = int(rndy / dsr)
        rndcoords = coords_lvl_vls[0, rnddsy, rnddsx, :, :, :].detach().cpu().view(81, 2).numpy() * dsr
        rndinitialfx = initflow[0, 0, rndy, rndx].detach().cpu().numpy().item()
        rndinitialfy = initflow[0, 1, rndy, rndx].detach().cpu().numpy().item()

        rndmigratesx = list()
        rndmigratesy = list()
        for k in range(len(flowpred)):
            rndmigratesx.append(flowpred[k][0, 0, rndy, rndx].item() + rndx)
            rndmigratesy.append(flowpred[k][0, 1, rndy, rndx].item() + rndy)

        rndprob = prob_vls[0, :, int(rndy / dsr), int(rndx / dsr)].detach().cpu().numpy()
        cm = plt.get_cmap('magma')
        rndcolor = cm(rndprob)[:, 0:3]

        flowgtx = flownp[0, rndy, rndx]
        flowgty = flownp[1, rndy, rndx]

        rndpts3d = np.array([[rndx * rndd, rndy * rndd, rndd, 1]]).T
        rndpts2d = rndP @ rndpts3d
        rndpts2dx = rndpts2d[0] / rndpts2d[2]
        rndpts2dy = rndpts2d[1] / rndpts2d[2]
        ...
        plt.scatter(rndpts2dx, rndpts2dy, 10, 'r')
        plt.scatter(rndx + flowgtx, rndy + flowgty, 10, 'r')
        plt.scatter(rndcoords[:, 0], rndcoords[:, 1], 3, 'g')
        plt.scatter(rndx + rndinitialfx, rndy + rndinitialfy, 20, 'g')
        plt.scatter(rndmigratesx, rndmigratesy, 10, 'b')
        plt.imshow(img2)

        fig.add_subplot(3, 1, 3)
        plt.scatter(rndcoords[:, 0], rndcoords[:, 1], 3, rndcolor)
        plt.scatter(rndmigratesx[1], rndmigratesy[1], 3, 'r')
        plt.imshow(img2)
...
def validate_VRKitti2(model, args, eval_loader, iters, group, logger, total_steps):
    ...
    for val_id, data_blob in enumerate(tqdm(eval_loader)):
        ...
        pMImg = data_blob['pMImg'].cuda(gpu, non_blocking=True)

        # exlude invalid pixels and extremely large diplacements
        mag = torch.sum(flow_gt ** 2, dim=1).sqrt()
        valid = ((flow_gt[:, 0] != 0) * (flow_gt[:, 1] != 0) * (mag < MAX_FLOW)).unsqueeze(1).float()
        mDdoutputs, flow_low, flow_pr = model(image1, image2, pMImg, iters=iters, test_mode=True)
        ...
        if not(logger is None) and np.mod(val_id, 20) == 0:
            logger.write_vls_eval(data_blob, flow_pr, mDdoutputs, valid, val_id, total_steps)
...
def train(gpu, ngpus_per_node, args):
    ...
    train_dataset = VirtualKITTI2(args=args, root=args.dataset_root, isapproxpose=False, inheight=args.inheight, inwidth=args.inwidth, entries=train_entries, istrain=True)
    ...
    eval_dataset = VirtualKITTI2(args=args, root=args.dataset_root, isapproxpose=False, inheight=args.evalheight, inwidth=args.evalwidth, entries=evaluation_entries, istrain=False)
    ...
    while should_keep_training:
        ...
        for i_batch, data_blob in enumerate(train_loader):
            ...
            pMImg = data_blob['pMImg'].cuda(gpu, non_blocking=True)
            depthgt = data_blob['depthmap'].cuda(gpu, non_blocking=True)
            ...
            mDdoutputs, initflow, flow_predictions, coords_lvl_vls, prob_vls = model(image1, image2, pMImg, iters=args.iters)

            depthselector = ((insmap > -1) * (depthgt < args.max_depth_pred) * (depthgt > args.min_depth_pred)).float()
            loss_depth = 0
            for k in range(4):
                depth_pred = F.interpolate(mDdoutputs[('mDepth', k)], [args.inheight, args.inwidth], mode='bilinear', align_corners=False)
                loss_depth += torch.sum(torch.abs(depth_pred - depthgt) * depthselector) / (torch.sum(depthselector) + 1)
            loss_depth = loss_depth / 4

            loss_flow, metrics = sequence_loss(flow_predictions, flow, valid, args.gamma)

            metrics = dict()
            metrics['loss_flow'] = loss_flow.float().item()
            metrics['loss_depth'] = loss_depth.float().item()

            loss = loss_depth + loss_flow
            ...
            if args.gpu == 0:
                ...
                if total_steps % SUM_FREQ == 0:
                    ...
                    print("Step: %d, rest hour: %f, flow loss: %f, depth loss: %f" % (total_steps, resths, loss_flow.item(), loss_depth.item()))
                    X = logger.vls_sampling(image1, image2, depthgt, initflow, pMImg, flow, flow_predictions, coords_lvl_vls, prob_vls)
                    logger.write_vls(data_blob, flow_predictions, mDdoutputs, valid, total_steps, X=X)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--samplewindowr', type=float, default=1)
