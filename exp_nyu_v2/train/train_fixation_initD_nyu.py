...
@torch.no_grad()
def validate_kitti(model, args, eval_loader, logger, group, total_steps, isorg=False, domask=False):
    ...
    for val_id, data_blob in enumerate(tqdm(eval_loader)):
        ...
        if not(logger is None) and np.mod(val_id, 20) == 0 and not isorg and not domask:
...
def train(gpu, ngpus_per_node, args):
    ...
    train_dataset = NYUV2(root=args.dataset_root, inheight=args.inheight, inwidth=args.inwidth, entries=train_entries, mdPred_root=args.mdPred_root,
                                RANSACPose_root=args.RANSACPose_root, flowpred_root=args.flowpred_root, istrain=True, ban_static_in_trianing=args.ban_static_in_trianing)
    ...
    if args.gpu == 0:
        ...
        logger_evaluation = Logger(os.path.join(args.logroot, 'evaluation_nyuv2', args.name))
        logger_evaluation_org = Logger(os.path.join(args.logroot, 'evaluation_nyuv2', "{}_org".format(args.name)))
    ...
    while should_keep_training:
        ...
        for i_batch, data_blob in enumerate(train_loader):
            ...
            if total_steps % VAL_FREQ == 1:
                ...
                if args.gpu == 0:
                    validate_kitti(model.module, args, eval_loader, logger, group, total_steps, isorg=False, domask=True)
                else:
                    validate_kitti(model.module, args, eval_loader, None, group, None, isorg=False, domask=True)
