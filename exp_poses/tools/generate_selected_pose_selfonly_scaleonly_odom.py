...
def read_splits(args):
    ...
    tot_entries = list(set(odom_entries))
    tot_entries.sort()
    return tot_entries
...
if __name__ == '__main__':
    ...
    for kkk in range(len(checkpoints)):
        args.restore_ckpt = checkpoints[kkk]
        ...
        if best_performance > tot_err['positions_pred'] / len(valentries):
            best_performance = tot_err['positions_pred'] / len(valentries)
            best_pth_name = checkpoints[kkk].split('/')[-1].split('.')[0]
        print("*******************************")
        print("Best pth is %s, performance: %f" % (best_pth_name, best_performance))
        print("*******************************")