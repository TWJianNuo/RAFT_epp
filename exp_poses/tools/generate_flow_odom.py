...
def get_odomentries(args):
    ...
    for odomseq in odomseqs:
        leftimgs = glob.glob(os.path.join(args.odom_root, odomseq[0:10], odomseq+'_sync', 'image_02/data', "*.png"))
        for leftimg in leftimgs:
            imgname = os.path.basename(leftimg)
            odomentries.append("{} {} {}".format("{}/{}".format(odomseq[0:10], odomseq+'_sync'), imgname.rstrip('.png'), 'l'))
    return odomentries
...
def read_splits(args):
    ...
    for entry in tot_entries:
        ...
        if not os.path.exists(os.path.join(args.exportroot, seq, 'image_02', str(frmidx).zfill(10) + '.png')):
            togen.append(entry)
    return togen
