...
class Scalehead(nn.Module):
    def __init__(self, args):
        ...
        self.convd1 = nn.Conv2d(in_channels=32, out_channels=32, kernel_size=3, stride=2, padding=1)
        self.convd2 = nn.Conv2d(in_channels=32, out_channels=32, kernel_size=3, stride=2, padding=1)
        self.convd3 = nn.Conv2d(in_channels=32, out_channels=32, kernel_size=3, stride=2, padding=1)
    ...
    def forward(self, x):
        ...
        x = self.relu(self.convd1(x))
        x = self.relu(self.convd2(x))
        x = self.relu(self.convd3(x))

        xs = torch.split(x, dim=1, split_size_or_sections=int(32 / self.args.num_angs))
        pred = list()
        for xx in xs:
            xx = self.conv4(xx)
            xx = xx.mean(dim=2).mean(dim=2)
            pred.append((self.sigmoid(xx) - 0.5) * 2 * self.args.maxlogscale)
        pred = torch.cat(pred, dim=1).unsqueeze(-1)
        return pred
...
class Relposehead(nn.Module):
    ...
    def forward(self, x):
        ...
        pred = self.softmax(xx).unsqueeze(-1)
        return pred
