...
class KITTI_eigen(data.Dataset):
    def __init__(self, entries, inheight, inwidth, maxinsnum, root, odom_root, ins_root, depth_root=None, depthvls_root=None, mdPred_root=None, num_samples=4, linlogdedge=None,
                 prediction_root=None, istrain=True, muteaug=False, isgarg=False, banremovedup=False, deepv2dpred_root=None, flowPred_root=None, RANSACPose_root=None, span=12):
        ...
        for entry in self.remove_dup(entries):
            ...
            img1path = os.path.join(tmp_root, seq, 'image_02', 'data', "{}.png".format(str(index).zfill(10)))
            img2path = os.path.join(tmp_root, seq, 'image_02', 'data', "{}.png".format(str(index + 1).zfill(10)))
            depthvlspath = os.path.join(depthvls_root, seq, 'image_02', "{}.png".format(str(index).zfill(10)))

            # Load Intrinsic for each frame
            calib_dir = os.path.join(tmp_root, seq.split('/')[0])
            ...
            access_key = seq.split('/')[1][0:21]
            if index >= int(seqmap[access_key]['stid']) and index < int(seqmap[access_key]['enid']):
                gt_relpose = self.odom_gt_relpose_rec[access_key][index - int(seqmap[access_key]['stid'])]
            else:
                gt_relpose = None
        ...
        # Read all IMU poses and RANSAC poses into memory
        ...
        self.IMU_poses_rec = dict()
        self.RANSAC_poses_rec = dict()
        for seq in all_seqs:
            pngs = glob(os.path.join(self.root, seq, 'image_02/data', '*.png'))
            ...
            IMU_poses = list()
            RANSAC_poses = list()
            for k in range(len(pngs)):
                if k == len(pngs) - 1:
                    IMU_poses.append(np.eye(4))
                else:
                    IMU_poses.append(get_pose(root, seq, k, extrinsic))
                RANSACPose_path = os.path.join(RANSACPose_root, "{}", seq, 'image_02', "{}.pickle".format(str(k).zfill(10)))
                RANSACPose = pickle.load(open(RANSACPose_path.format(str(0).zfill(3)), "rb"))
                RANSAC_poses.append(RANSACPose[0])

            self.IMU_poses_rec[seq.split('/')[1]] = IMU_poses
            self.RANSAC_poses_rec[seq.split('/')[1]] = RANSAC_poses
    ...
    def __getitem__(self, index):
        ...
        if self.gt_relpose_rec[index] is None:
            rel_pose_odom = copy.deepcopy(rel_pose)
        else:
            rel_pose_odom = copy.deepcopy(self.gt_relpose_rec[index])

        inspred = np.array(Image.open(self.inspred_list[index])).astype(np.int)

        self.npossibility = len(glob(os.path.join(self.RANSACPose_root, '*'))) - 1
        ...
        data_blob = self.wrapup(img1=img1, img2=img2, flowgt=flowgt, flowgt_vls=flowgt_vls, depthmap=depth, depthvls=depthvls,
                                depthpred_deepv2d=depthpred_deepv2d, mdDepth_pred=mdDepth_pred, intrinsic=intrinsic, insmap=inspred, flowpred=flowpred_RAFT, rel_pose=rel_pose, rel_pose_odom=rel_pose_odom,
    ...
    def wrapup(self, img1, img2, flowgt, flowgt_vls, depthmap, depthvls, depthpred_deepv2d, mdDepth_pred, intrinsic, insmap, flowpred, rel_pose, rel_pose_odom, posepred, posepred_deepv2d, ang_decps_pad, scl_decps_pad, mvd_decps_pad, tag):
        ...
        data_blob['rel_pose_odom'] = rel_pose_odom
