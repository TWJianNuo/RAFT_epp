...
def depth2scale(pts2d1, pts2d2, intrinsic, R, t, coorespondedDepth):
    ...
    rel_d = np.sqrt(((delta_t[0] - pts2d2[0, selector] * delta_t[2]) ** 2 + (delta_t[1] - pts2d2[1, selector] * delta_t[2]) ** 2) / denom[selector])
    alpha = np.mean(coorespondedDepth[selector]) / np.mean(rel_d)
    return alpha

def depth2scale_abs(pts2d1, pts2d2, intrinsic, R, t, coorespondedDepth):
    intrinsic33 = intrinsic[0:3, 0:3]
    M = intrinsic33 @ R @ np.linalg.inv(intrinsic33)
    delta_t = (intrinsic33 @ t).squeeze()
    minval = 1e-6
    ...
    denom = np.abs(pts2d2[0, :] * (np.expand_dims(M[2, :], axis=0) @ pts2d1).squeeze() - (np.expand_dims(M[0, :], axis=0) @ pts2d1).squeeze()) + \
            np.abs(pts2d2[1, :] * (np.expand_dims(M[2, :], axis=0) @ pts2d1).squeeze() - (np.expand_dims(M[1, :], axis=0) @ pts2d1).squeeze())

    selector = (denom > minval)
    if np.sum(selector) == 0:
        return 0

    rel_d = ((np.abs(delta_t[0] - pts2d2[0, selector] * delta_t[2]) + np.abs(delta_t[1] - pts2d2[1, selector] * delta_t[2])) / denom[selector])
    alpha = np.mean(coorespondedDepth[selector]) / np.mean(rel_d)
    return alpha

def depth2scale_mean(pts2d1, pts2d2, intrinsic, R, t, coorespondedDepth):
    intrinsic33 = intrinsic[0:3, 0:3]
    M = intrinsic33 @ R @ np.linalg.inv(intrinsic33)
    delta_t = (intrinsic33 @ t).squeeze()
    minval = 1e-6

    denom1 = pts2d2[0, :] * (np.expand_dims(M[2, :], axis=0) @ pts2d1).squeeze() - (np.expand_dims(M[0, :], axis=0) @ pts2d1).squeeze()
    denom2 = pts2d2[1, :] * (np.expand_dims(M[2, :], axis=0) @ pts2d1).squeeze() - (np.expand_dims(M[1, :], axis=0) @ pts2d1).squeeze()

    selector = (denom1 > minval) * (denom2 > minval)

    if np.sum(selector) == 0:
        return 0

    rel_d1 = np.abs(delta_t[0] - pts2d2[0, selector] * delta_t[2]) / denom1[selector]
    rel_d2 = np.abs(delta_t[1] - pts2d2[1, selector] * delta_t[2]) / denom2[selector]

    alpha = np.mean(coorespondedDepth[selector]) / np.mean((rel_d1 + rel_d2) / 2)
    return alpha
...
def inf_pose_flow(flow_pr_inf, insmap, mdDepth, intrinsic, pid, gradComputer=None, banins=False, samplenum=50000, args=None):
    ...
    if args.org :
        ...
        scale_md = depth2scale_abs(pts1_inliers, pts2_inliers, intrinsicnp, R, t, mdDepth_np[selector][rndidx][inliers_mask])
    elif args.mean:
        scale_md = depth2scale_mean(pts1_inliers, pts2_inliers, intrinsicnp, R, t, mdDepth_np[selector][rndidx][inliers_mask])
    else:
        raise Exception("select one")

    if R[0, 0] < 0 or R[1, 1] < 0 or R[2, 2] < 0 or t[2] > 0 or scale_md == 0:
        R = np.eye(3)
        t = np.array([[0, 0, -1]]).T
        scale_md = 0
...
def validate_RANSAC_odom_relpose(args, eval_loader, banins=False, bangrad=False, samplenum=50000):
    ...
    foldname = "banins_{}_bangrad_{}_samplenum_{}_{}".format(banins, bangrad, samplenum, attstr)
    ...
    for val_id, data_blob in enumerate(tqdm(eval_loader)):
        ...
        if torch.sum(mdDepth_pred <= 0) > 0:
            print("invalid, skip")
            continue
        tag = data_blob['tag']

        R, t, scale, flow_sel_mag = inf_pose_flow(flowpred, insmap, mdDepth_pred, intrinsic, int(val_id + 10), gradComputer=gradComputer, banins=banins, samplenum=samplenum, args=args)
...
def generate_seqmapping():
    ...
    return seqmap, test_entries + entries
