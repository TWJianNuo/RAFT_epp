...
def get_seq_pos(poses, stidx, edidx, reverse=False):
    assert stidx >= 0 and edidx <= len(poses) and edidx > stidx
    accumM = np.eye(4)
    accumpos = list()
    if not reverse:
        for k in range(stidx, edidx):
            accumM = poses[k][0].numpy() @ accumM
            accumpos.append(np.linalg.inv(accumM)[0:3, 3])
    else:
        for k in range(edidx - 1, stidx - 1, -1):
            accumM = np.linalg.inv(poses[k][0].numpy()) @ accumM
            accumpos.append(np.linalg.inv(accumM)[0:3, 3])

    return np.array(accumpos)
...
def get_coord_loss(IMUPose, RANSACPose, idx, estpose, span=50):
    ...
    if stidx < 0:
        stidx = 0
    if edidx > len(IMUPose):
        edidx = len(IMUPose)

    tmpRANSACPose = copy.deepcopy(RANSACPose)
    tmpRANSACPose[idx] = estpose.float().unsqueeze(0)

    accumpos_IMU_pos = get_seq_pos(IMUPose, stidx, edidx, reverse=False)
    accumpos_IMU_neg = get_seq_pos(IMUPose, stidx, edidx, reverse=True)

    accumpos_RANSAC_pos = get_seq_pos(tmpRANSACPose, stidx, edidx, reverse=False)
    accumpos_RANSAC_neg = get_seq_pos(tmpRANSACPose, stidx, edidx, reverse=True)

    loss = np.mean(np.abs(accumpos_IMU_pos - accumpos_RANSAC_pos)) + np.mean(np.abs(accumpos_IMU_neg - accumpos_RANSAC_neg))

    # plt.figure()
    # plt.scatter(accumpos_IMU_pos[:, 0], accumpos_IMU_pos[:, 2], 5)
...
def validate_RANSAC_odom_corr(span, args, eval_loader, banins=False, bangrad=False, samplenum=50000):
    print("Initialize Pid: %d, Span: %d" % (span, span * 2))
    span = int(span * 2)
    if bangrad:
        gradComputer = None
    else:
    ...
    for val_id, data_blob in enumerate(tqdm(eval_loader)):
        ...
        for k in range(trialtime):
            ...
            accumerr = get_coord_loss(IMUPose, RANSACPose, int(tag.split(' ')[1]), self_pose, span=span)
            ...
            # tensor2rgb(rgb1, viewind=0).show()
            flow_loss_rec.append(flow_loss.item())
            accumerr_rec.append(accumerr.item())

        r, _ = pearsonr(np.array(flow_loss_rec), np.array(accumerr_rec))
        corrlist.append(r)

    plt.figure()
    plt.hist(corrlist, bins=200, range=(-1, 1))
    plt.title("Coor : %f" % np.mean(np.array(corrlist)))
    plt.savefig(os.path.join(args.vls_root, 'coor_accum_span_{}.png'.format(str(span).zfill(2))))
    plt.close()
...
def validate_RANSAC_odom_corr_bs(args, eval_loader, banins=False, bangrad=False, samplenum=50000):
    ...
    plt.title("Ang coor: %f" % (np.mean(np.array(corrlist_ang))))
    fig.add_subplot(2, 1, 2)
    plt.hist(corrlist_t, bins=100, range=(-1, 1))
    plt.title("T coor: %f" % (np.mean(np.array(corrlist_t))))
