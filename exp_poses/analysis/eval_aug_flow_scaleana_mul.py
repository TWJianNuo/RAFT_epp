...
@torch.no_grad()
def validate_RANSAC_poses_mul_vls(args, eval_loader, group, num_frames=5, dovls=False):
    gpu = args.gpu
    eval_metrics = {'pixelnum': torch.zeros(1).cuda(device=gpu), 'framenum': torch.zeros(1).cuda(device=gpu)}

    maxl = 0.2
    numaba = 8
    scales_pos = np.linspace(0, maxl, int(numaba / 2 + 1))
    scales = np.concatenate([scales_pos, -scales_pos[1::]], axis=0)
    scales = np.sort(scales)

    for k in range(numaba):
        eval_metrics["pose_RANSAC_scale_%d" % k] = torch.zeros(num_frames).cuda(device=gpu)
    ...
    for val_id, data_blob in enumerate(tqdm(eval_loader)):
        ...
        for k in range(numaba):
            ...
            tmpscale = np.exp(np.log(scale_md) + scales[k])
            if istatic:
                ...
                pose_RANSAC[0, 0, 0:3, 3:4] = torch.from_numpy(t * tmpscale).float().cuda(intrinsic.device)

            poses["pose_RANSAC_scale_%d" % k] = pose_RANSAC
        ...
        for idx, k in enumerate(poses.keys()):
            ...
            if dovls:
                ...
                vlsh = int(idx / 2)
                vlsw = int(idx - vlsh * 2)
                titlestr = "%s, Performance Level %f" % (k, (torch.sum(out).item() / out.shape[0]))
                axs[vlsh][vlsw].scatter(xxval, yyval, 0.01, 'b')
                axs[vlsh][vlsw].scatter(xxval[outnp], yyval[outnp], 1, 'r')
                axs[vlsh][vlsw].imshow(rgbvls)
                axs[vlsh][vlsw].title.set_text(titlestr)
...
def train(gpu, ngpus_per_node, args):
    ...
    validate_RANSAC_poses_mul_vls(args, eval_loader, group, num_frames=5, dovls=True)
    return
