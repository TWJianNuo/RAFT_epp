...
def inf_pose_flow(img1, img2, flow_pr_inf, insmap, depthmap, mdDepth, intrinsic, pid, gradComputer=None):
...
@torch.no_grad()
def validate_RANSAC_poses(args, eval_loader, group, num_frames=5, dovls=False):
    gpu = args.gpu
    ...
    for val_id, data_blob in enumerate(tqdm(eval_loader)):
        ...
        R, t, scale, scale_md, flow_sel_mag, istatic = inf_pose_flow(image1, image2, flowpred, insmap, depthgt, mdDepth_pred, intrinsic, int(val_id + args.gpu * 1000 + 10), gradComputer=None)
        ...
        for idx, k in enumerate(poses.keys()):
            ...
            if dovls:
                ...
                if k == 'pose_RANSAC':
                    titlestr = "%s, Performance Level %f, scale_gt: %.4f, scale_dd: %.4f" % (k, (torch.sum(out).item() / out.shape[0]), scale, scale_md)
                else:
                    titlestr = "%s, Performance Level %f, scale_gt: %.4f, scale_md: %.4f" % (k, (torch.sum(out).item() / out.shape[0]), scale, torch.sqrt(torch.sum(posepred_deepv2d[0, 0, 0:3, 3:4] ** 2)).item())
...
@torch.no_grad()
def validate_RANSAC_scalecontrast(args, eval_loader, group, num_frames=5, dovls=False):
    gpu = args.gpu
    eval_metrics = {'pose_RANSAC_mdscale': torch.zeros(1).cuda(device=gpu), 'pose_RANSAC': torch.zeros(num_frames).cuda(device=gpu), 'pixelnum': torch.zeros(1).cuda(device=gpu),
                    'framenum': torch.zeros(1).cuda(device=gpu)}
    ...
    for val_id, data_blob in enumerate(tqdm(eval_loader)):
        ...
        if istatic:
            pose_RANSAC[0, 0, :, :] = torch.eye(4, device=intrinsic.device)
            pose_RANSAC_mdscale[0, 0, :, :] = torch.eye(4, device=intrinsic.device)
        else:
            pose_RANSAC[0, 0, 0:3, 0:3] = torch.from_numpy(R).float().cuda(intrinsic.device)
            pose_RANSAC[0, 0, 0:3, 3:4] = torch.from_numpy(t * scale).float().cuda(intrinsic.device)
            pose_RANSAC_mdscale[0, 0, 0:3, 0:3] = torch.from_numpy(R).float().cuda(intrinsic.device)
            pose_RANSAC_mdscale[0, 0, 0:3, 3:4] = torch.from_numpy(t * scale_md).float().cuda(intrinsic.device)

        poses = {'pose_RANSAC_mdscale': pose_RANSAC_mdscale, 'pose_RANSAC': pose_RANSAC}
        ...
        for idx, k in enumerate(poses.keys()):
            ...
            if dovls:
                ...
                if k == 'pose_RANSAC':
                    titlestr = "%s, Performance Level %f, scale_gt: %.4f" % (k, (torch.sum(out).item() / out.shape[0]), scale)
                else:
                    titlestr = "%s, Performance Level %f, reldiff: %.4f" % (k, (torch.sum(out).item() / out.shape[0]), np.abs(scale_md - scale) / scale)
                axs[idx].scatter(xxval, yyval, 0.01, 'b')
                axs[idx].scatter(xxval[outnp], yyval[outnp], 1, 'r')
                axs[idx].imshow(rgbvls)
                axs[idx].title.set_text(titlestr)
...
def train(gpu, ngpus_per_node, args):
    ...
    # validate_RANSAC_scalecontrast(args, eval_loader, group, num_frames=5, dovls=False)
    validate_RANSAC_poses(args, eval_loader, group, num_frames=5, dovls=False)
    return
