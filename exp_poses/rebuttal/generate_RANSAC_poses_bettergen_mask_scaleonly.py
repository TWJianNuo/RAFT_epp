...
class KITTI_eigen(data.Dataset):
    def __init__(self, entries, root='datasets/KITTI', odom_root=None, ins_root=None, flowPred_root=None, mdPred_root=None, bsposepred_root=None, mask_root=None, banins=False):
        ...
        for entry in self.remove_dup(entries):
            ...
            maskpath = os.path.join(mask_root, seq, "{}.pickle".format(str(index).zfill(10)))
            ...
            pose_bs_path = os.path.join(self.bsposepred_root, seq, 'image_02', "{}.pickle".format(str(index).zfill(10)))
        ...
        assert len(self.intrinsic_list) == len(self.entries) == len(self.image_list) == len(self.mask_list)
    ...
    def __getitem__(self, index):
        ...
        mask = pickle.load(open(self.mask_list[index], "rb"))
        assert img1.shape[0:2] == mask.shape
        ...
        data_blob = self.wrapup(img1=img1, img2=img2, intrinsic=intrinsic, insmap=inspred, mdDepth=mdDepth, flowpred_RAFT=flowpred_RAFT, mask=mask, posepred_bs=posepred_bs, tag=self.entries[index])
        return data_blob

    def wrapup(self, img1, img2, intrinsic, insmap, mdDepth, flowpred_RAFT, mask, posepred_bs, tag):
...
def inf_pose_flow(pose_ref, flow_pr_inf, insmap, mdDepth, mask, intrinsic, pid, gradComputer=None, banins=False, samplenum=50000):
    pose_ref_np = pose_ref.numpy()
    pose_ref_normed = copy.deepcopy(pose_ref_np)
    pose_ref_normed_scale = np.sqrt(np.sum(pose_ref_normed[0:3, 3] ** 2) + 1e-10)
    pose_ref_normed[0:3, 3] = pose_ref_normed[0:3, 3] / pose_ref_normed_scale
    ...
    selector = selector == 1
    # tensor2disp(torch.from_numpy(mask).unsqueeze(0).unsqueeze(0), vmax=1, viewind=0).show()
    # tensor2disp(1 / torch.from_numpy(mdDepth_np).unsqueeze(0).unsqueeze(0), vmax=0.15, viewind=0).show()

    np.random.seed(pid)
    xx_idx_sel = xx[selector]
    yy_idx_sel = yy[selector]

    topsorted = mask[yy_idx_sel, xx_idx_sel]
    descendidx = np.argsort(-topsorted)
    if descendidx.shape[0] < samplenum:
        samplenum = descendidx.shape[0]
        if samplenum < 100:
            return pose_ref_np
    yy_idx_sel = yy_idx_sel[descendidx[0:samplenum]]
    xx_idx_sel = xx_idx_sel[descendidx[0:samplenum]]
    ...
    scale_md = depth2scale(pts1_inliers, pts2_inliers, intrinsicnp, pose_ref_normed[0:3, 0:3], pose_ref_normed[0:3, 3:4], mdDepth_np[yy_idx_sel, xx_idx_sel][inliers_mask])
    if not (scale_md > 0 and scale_md < 10):
        print(scale_md)
        return pose_ref_np
    pose_ref_normed[0:3, 3:4] = pose_ref_normed[0:3, 3:4] * scale_md
...
def validate_RANSAC_odom_relpose(args, eval_loader, banins=False, bangrad=False, samplenum=50000, iters=0):
    ...
    for val_id, data_blob in enumerate(tqdm(eval_loader)):
        ...
        if torch.sum(torch.abs(data_blob['img1'] - data_blob['img2'])) < 1:
            pose_ref_normed = np.eye(4)
        else:
            pose_ref_normed = inf_pose_flow(pose_bs[0, 0], flowpred, insmap, mdDepth_pred, mask, intrinsic, int(iters * eval_loader.__len__() + val_id), gradComputer=gradComputer, banins=banins, samplenum=samplenum)

        pose_ref_normed = np.expand_dims(pose_ref_normed, axis=0)

        with open(export_root, 'wb') as handle:
            pickle.dump(pose_ref_normed, handle, protocol=pickle.HIGHEST_PROTOCOL)
...
def train(processid, args, entries, iters=0):
    ...
    eval_dataset = KITTI_eigen(root=args.dataset_root, odom_root=args.odom_root, entries=entries[stidx : edidx],  flowPred_root=args.flowPred_root,
                               mdPred_root=args.mdPred_root, ins_root=args.ins_root, bsposepred_root=args.bsposepred_root, mask_root=args.mask_root, banins=args.banins)
