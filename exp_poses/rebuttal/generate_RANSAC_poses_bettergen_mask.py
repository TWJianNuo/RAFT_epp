...
def inf_pose_flow(flow_pr_inf, insmap, mdDepth, mask, intrinsic, pid, gradComputer=None, banins=False, samplenum=50000):
    ...
    scale_md = depth2scale(pts1_inliers, pts2_inliers, intrinsicnp, R, t, mdDepth_np[yy_idx_sel, xx_idx_sel][inliers_mask])
...
def validate_RANSAC_odom_relpose(args, eval_loader, banins=False, bangrad=False, samplenum=50000, iters=0):
    ...
    for val_id, data_blob in enumerate(tqdm(eval_loader)):
        ...
        if torch.sum(torch.abs(data_blob['img1'] - data_blob['img2'])) < 1:
            ...
            R, t, scale = inf_pose_flow(flowpred, insmap, mdDepth_pred, mask, intrinsic, int(iters * eval_loader.__len__() + val_id), gradComputer=gradComputer, banins=banins, samplenum=samplenum)
        self_pose = np.eye(4)
        self_pose[0:3, 0:3] = R
