...
def validate_kitti_colorjitter(gpu, model, args, ngpus_per_node, eval_entries, iters=24):
    ...
    with torch.no_grad():
        for val_id, entry in enumerate(tqdm(remove_dup(eval_entries[stidx : edidx]))):
            ...
            svfold_forward = os.path.join(args.exportroot, seq, 'image_02/forward')
            svfold_backward = os.path.join(args.exportroot, seq, 'image_02/backward')
            os.makedirs(svfold_forward, exist_ok=True)
            os.makedirs(svfold_backward, exist_ok=True)

            svpath_forward = os.path.join(svfold_forward, "{}.png".format(str(index).zfill(10)))
            svpath_backward = os.path.join(svfold_backward, "{}.png".format(str(index).zfill(10)))
            ...
            frame_utils.writeFlowKITTI(svpath_forward, flow.permute(1, 2, 0).numpy())

            flow_low, flow_pr = model(image2, image1, iters=iters, test_mode=True)
            flow = padder.unpad(flow_pr[0]).cpu()
            frame_utils.writeFlowKITTI(svpath_backward, flow.permute(1, 2, 0).numpy())
    return
