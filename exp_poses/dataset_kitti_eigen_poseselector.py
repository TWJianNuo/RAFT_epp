...
class KITTI_eigen(data.Dataset):
    ...
    def wrapup(self, img1, img2, flowgt, flowgt_vls, depthmap, depthvls, depthpred_deepv2d, mdDepth_pred, intrinsic, insmap, flowpred, rel_pose, posepred, posepred_deepv2d, ang_decps_pad, scl_decps_pad, mvd_decps_pad, tag,
               IMUlocations1, leftarrs1, rightarrs1, IMUlocations2, leftarrs2, rightarrs2):
        ...
        IMUlocations1 = torch.from_numpy(IMUlocations1).float()
        data_blob['IMUlocations1'] = IMUlocations1
        leftarrs1 = torch.from_numpy(leftarrs1).float()
        data_blob['leftarrs1'] = leftarrs1
        rightarrs1 = torch.from_numpy(rightarrs1).float()
        data_blob['rightarrs1'] = rightarrs1
        IMUlocations2 = torch.from_numpy(IMUlocations2).float()
        data_blob['IMUlocations2'] = IMUlocations2
        leftarrs2 = torch.from_numpy(leftarrs2).float()
        data_blob['leftarrs2'] = leftarrs2
        rightarrs2 = torch.from_numpy(rightarrs2).float()
        data_blob['rightarrs2'] = rightarrs2
