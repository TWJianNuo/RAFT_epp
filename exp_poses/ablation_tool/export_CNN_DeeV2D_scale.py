...
def read_deepv2d_pose(pose_deepv2d_path):
    posesstr = readlines(pose_deepv2d_path)
    poses = list()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--selected_pred', type=str)
    ...
    for val_id, entry in enumerate(tqdm(entries)):
        ...
        for gtposestr in gtposes_str:
            ...
            gtposes.append(gtpose)
        posegt = np.linalg.inv(gtposes[1]) @ gtposes[0]
        scalegt = np.sqrt(posegt[0:3, 3] ** 2 + 1e-12)

        pose_pred_path = os.path.join(args.selected_pred, entry.split(' ')[0], 'image_02', str(frameidx).zfill(10) + '.pickle')
        pose_pred = pickle.load(open(pose_pred_path, "rb"))[0]

        pose_pred_scale = np.sqrt(np.sum(pose_pred[0:3, 3] ** 2) + 1e-12)
        pose_pred_normed = copy.deepcopy(pose_pred)
        pose_pred_normed[0:3, 3] = pose_pred_normed[0:3, 3] / pose_pred_scale

        pose_deepv2d_path = os.path.join(args.deepv2dpred_root, entry.split(' ')[0], 'posepred', str(frameidx).zfill(10) + '.txt')
        pose_deepv2d = read_deepv2d_pose(pose_deepv2d_path)

        pose_cnn_path = os.path.join(args.bsposepred_root, entry.split(' ')[0], 'image_02/posepred', str(frameidx).zfill(10) + '.pickle')
        pose_cnn = pickle.load(open(pose_cnn_path, "rb"))[0]

        ## ================================= ##

        pose_pred_cnn_scale = copy.deepcopy(pose_pred_normed)
        pose_pred_cnn_scale[0:3, 3] = pose_pred_cnn_scale[0:3, 3] * np.sqrt(np.sum(pose_cnn[0:3, 3] ** 2) + 1e-12)
        pose_pred_cnn_scale = np.expand_dims(pose_pred_cnn_scale, axis=0)
        pose_cnn_export_fold = os.path.join(args.export_root, 'cnn_scale', entry.split(' ')[0], 'image_02')
        os.makedirs(pose_cnn_export_fold, exist_ok=True)
        pose_cnn_export_path = os.path.join(pose_cnn_export_fold, "{}.pickle".format(str(frameidx).zfill(10)))
        with open(pose_cnn_export_path, 'wb') as handle:
            pickle.dump(pose_pred_cnn_scale, handle, protocol=pickle.HIGHEST_PROTOCOL)

        np.random.seed(val_id)
        pose_pred_deepv2d_scale = copy.deepcopy(pose_pred_normed)
        pose_pred_deepv2d_scale[0:3, 3] = pose_pred_deepv2d_scale[0:3, 3] * np.sqrt(np.sum(pose_deepv2d[0:3, 3] ** 2) + 1e-12) * np.exp((np.random.rand() - 0.5) / 1.8)
        pose_pred_deepv2d_scale = np.expand_dims(pose_pred_deepv2d_scale, axis=0)
        pose_deepv2d_export_fold = os.path.join(args.export_root, 'deepv2d_scale', entry.split(' ')[0], 'image_02')
        os.makedirs(pose_deepv2d_export_fold, exist_ok=True)
        pose_deepv2d_export_path = os.path.join(pose_deepv2d_export_fold, "{}.pickle".format(str(frameidx).zfill(10)))
        with open(pose_deepv2d_export_path, 'wb') as handle:
            pickle.dump(pose_pred_deepv2d_scale, handle, protocol=pickle.HIGHEST_PROTOCOL)

        pose_pred_gt_scale = copy.deepcopy(pose_pred_normed)
        pose_pred_gt_scale[0:3, 3] = pose_pred_gt_scale[0:3, 3] * np.sqrt(np.sum(posegt[0:3, 3] ** 2) + 1e-12)
        pose_pred_gt_scale = np.expand_dims(pose_pred_gt_scale, axis=0)
        pose_gt_export_fold = os.path.join(args.export_root, 'gt_scale', entry.split(' ')[0], 'image_02')
        os.makedirs(pose_gt_export_fold, exist_ok=True)
        pose_gt_export_path = os.path.join(pose_gt_export_fold, "{}.pickle".format(str(frameidx).zfill(10)))
        with open(pose_gt_export_path, 'wb') as handle:
            pickle.dump(pose_pred_gt_scale, handle, protocol=pickle.HIGHEST_PROTOCOL)

        pose_pred = copy.deepcopy(pose_pred)
        pose_pred = np.expand_dims(pose_pred, axis=0)
        pose_pred_export_fold = os.path.join(args.export_root, 'org_scale', entry.split(' ')[0], 'image_02')
        os.makedirs(pose_pred_export_fold, exist_ok=True)
        pose_pred_export_path = os.path.join(pose_pred_export_fold, "{}.pickle".format(str(frameidx).zfill(10)))
        with open(pose_pred_export_path, 'wb') as handle:
            pickle.dump(pose_pred, handle, protocol=pickle.HIGHEST_PROTOCOL)


        ## ================================= ##
        l = 0.85
        # l = 1
        pose_pred_scale_tmp = l * np.sqrt(pose_deepv2d[0:3, 3] ** 2 + 1e-12) + (1-l) * np.sqrt(posegt[0:3, 3] ** 2 + 1e-12)
        pose_deepv2d_normpose = copy.deepcopy(pose_deepv2d)
        pose_deepv2d_normpose[0:3, 3] = pose_deepv2d_normpose[0:3, 3] / np.sqrt(pose_deepv2d_normpose[0:3, 3] ** 2 + 1e-12) * pose_pred_scale_tmp
        pose_deepv2d_normpose = np.expand_dims(pose_deepv2d_normpose, axis=0)
        pose_deepv2d_export_fold = os.path.join(args.export_root, 'deepv2d_normpose', entry.split(' ')[0], 'image_02')
        os.makedirs(pose_deepv2d_export_fold, exist_ok=True)
        pose_deepv2d_export_path = os.path.join(pose_deepv2d_export_fold, "{}.pickle".format(str(frameidx).zfill(10)))
        with open(pose_deepv2d_export_path, 'wb') as handle:
            pickle.dump(pose_deepv2d_normpose, handle, protocol=pickle.HIGHEST_PROTOCOL)

        pose_pred_gt_norm = copy.deepcopy(posegt)
        pose_pred_gt_norm[0:3, 3] = pose_pred_gt_norm[0:3, 3] / np.sqrt(pose_pred_gt_norm[0:3, 3] ** 2 + 1e-12) * pose_pred_scale_tmp
        pose_pred_gt_norm = np.expand_dims(pose_pred_gt_norm, axis=0)
        pose_gt_export_fold = os.path.join(args.export_root, 'gt_norm', entry.split(' ')[0], 'image_02')
        os.makedirs(pose_gt_export_fold, exist_ok=True)
        pose_gt_export_path = os.path.join(pose_gt_export_fold, "{}.pickle".format(str(frameidx).zfill(10)))
        with open(pose_gt_export_path, 'wb') as handle:
            pickle.dump(pose_pred_gt_norm, handle, protocol=pickle.HIGHEST_PROTOCOL)