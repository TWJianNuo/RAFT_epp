...
class KITTI_odom(data.Dataset):
    def __init__(self, entries, inheight, inwidth, maxinsnum, root, odomroot, ins_root, depth_root=None, mdPred_root=None, num_samples=4, linlogdedge=None,
        ...
        for entry in self.remove_dup(entries):
            ...
            if not os.path.exists(os.path.join(root, seq, 'image_02', 'data', "{}.png".format(str(index).zfill(10)))):
                croot = odomroot
            else:
                croot = root

            img1path = os.path.join(croot, seq, 'image_02', 'data', "{}.png".format(str(index).zfill(10)))
            img2path = os.path.join(croot, seq, 'image_02', 'data', "{}.png".format(str(index + 1).zfill(10)))
    ...
    def __getitem__(self, index):
        ...
        if self.RANSACPose_root is not None:
            ...
            for rep_idx in rep_idces:
                posepred_cur = pickle.load(open(self.RANSACPose_list[index], "rb"))
        ...
        img1, img2, depth, depthpred_deepv2d, mdDepth_pred, inspred, flowpred_RAFT, intrinsic = self.aug_crop(img1, img2, depth, depthpred_deepv2d, mdDepth_pred, inspred, flowpred_RAFT, intrinsic)
        ...
        data_blob = self.wrapup(img1=img1, img2=img2, flowgt=flowgt, depthmap=depth,
    ...
    def wrapup(self, img1, img2, flowgt, depthmap, depthpred_deepv2d, mdDepth_pred, intrinsic, insmap, flowpred, rel_pose, posepred, posepred_deepv2d, ang_decps_pad, scl_decps_pad, mvd_decps_pad, tag):
    ...
    def aug_crop(self, img1, img2, depthmap, depthpred_deepv2d, mdDepth_pred, instancemap, flowpred_RAFT, intrinsic):
        if img1.ndim == 3:
            h, w, _ = img1.shape
        else:
            h, w = img1.shape
        ...
        return img1, img2, depthmap, depthpred_deepv2d, mdDepth_pred, instancemap, flowpred_RAFT, intrinsic
