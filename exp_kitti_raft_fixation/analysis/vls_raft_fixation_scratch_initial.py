...
def validate_kitti(model, args, eval_loader):
    ...
    model.eval()
    gpu = args.gpu
    foldname = os.path.join(args.vlsroot, args.restore_ckpt.split('/')[-2], 'Depth')
    os.makedirs(foldname, exist_ok=True)
    foldname_grad = os.path.join(args.vlsroot, args.restore_ckpt.split('/')[-2], 'Grad')
    os.makedirs(foldname_grad, exist_ok=True)
    for val_id, data_blob in enumerate(tqdm(eval_loader)):
        ...
        outputs = model(image1, image2, intrinsic, posepred, insmap, iters=args.iters, initialdepth=depthpred)

        N, C, H, W = image1.shape
        init_depth = depthpred
        vlsimg = np.zeros([H * 4, W * 3, 3], dtype=np.uint8)

        depth_predictions = outputs['depth_predictions']
        for k in range(len(depth_predictions)):
            residual_depth = torch.log(depth_predictions[k]) - torch.log(init_depth)
            fig_residual = tensor2grad(residual_depth, pos_bar=0.1 / (1.5 ** k), neg_bar=-0.1 / (1.5 ** k), viewind=0)
            init_depth = depth_predictions[k]

            hid = int(k / 3)
            wid = int(k - hid * 3)
            vlsimg[hid * H : (hid+1) * H, wid * W:(wid+1) * W, :] = np.array(fig_residual)
        ...
        figpred = tensor2disp(1 / depth_prediction, vmax=0.15, viewind=0)
        figcp = tensor2disp(1 / depthpred, vmax=0.15, viewind=0)
        figcombined = np.concatenate([np.array(figpred), np.array(figcp)], axis=0)
        Image.fromarray(figcombined).save(os.path.join(foldname, svname))
        Image.fromarray(vlsimg).save(os.path.join(foldname_grad, svname))
