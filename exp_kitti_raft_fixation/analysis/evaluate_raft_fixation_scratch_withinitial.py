...
@torch.no_grad()
def validate_kitti(model, args, eval_loader, group, isorg=False):
    ...
    for val_id, data_blob in enumerate(tqdm(eval_loader)):
        ...
        selector = ((depth_prediction > 0) * (depthgt > args.min_depth_eval) * (depthgt < args.max_depth_eval)).float()
        depth_prediction = torch.clamp(depth_prediction, min=args.min_depth_eval, max=args.max_depth_eval)
